...
import type { UpdateMode } from "./update-form";
...
export const ReviewStep = forwardRef<ReviewStepHandle, ReviewStepProps>(
  function ReviewStep(
    { account, extractionResult, source, onCommitSuccess, onBack, onCommitDisabledChange },
    ref
  ) {
    ...
    return (
      <div className="flex flex-col gap-3">
                  ...
                  >
                    ...
                    <td className="px-2 py-1 min-w-[160px]">
                      <div className="flex flex-col gap-1">
                        <select
                          ...
                          onChange={(e) =>
                            updateRow(row.id, "asset_class", e.target.value || null)
                          }
                        >
                        ...
                        {row.showAdvanced && (
                          <div className="flex flex-col gap-1 pl-1 border-l-2 border-border">
                            <input
                              ...
                              value={row.sub_class ?? ""}
                              onChange={(e) =>
                                updateRow(row.id, "sub_class", e.target.value || null)
                              }
                            />
                            <input
                              className="rounded border border-input bg-background px-1.5 py-0.5 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                              ...
                              value={row.sector ?? ""}
                              onChange={(e) =>
                                updateRow(row.id, "sector", e.target.value || null)
                              }
                            />
                            <select
                              className="rounded border border-input bg-background px-1.5 py-0.5 text-xs focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                              value={row.region ?? ""}
                              onChange={(e) =>
                                updateRow(row.id, "region", e.target.value || null)
                              }
                            >
                    ...
                    <td className="px-2 py-1 text-right">
                      <input
                        ...
                        value={row.shares}
                        onChange={(e) =>
                          updateRow(row.id, "shares", Number(e.target.value))
                        }
                        disabled={row.status === "removed"}
                      />
                    </td>
                    ...
                    {/* Confidence */}
                    <td className="px-2 py-1.5 text-center text-muted-foreground text-xs tabular-nums">
                      {row.status === "removed" ? "—" : formatPct(row.confidence)}
                    </td>
                    ...
                    <td className="px-2 py-1.5 text-center">
                      {row.source_span ? (
                        <Popover>
                          ...
                          <PopoverContent className="max-w-xs text-xs font-mono whitespace-pre-wrap">
                            {row.source_span}
                          </PopoverContent>
                        </Popover>
                      ) : null}
        ...
        {/* Totals bar */}
        <div className="rounded-md border border-border bg-muted/20 px-3 py-2 text-body-sm tabular-nums flex flex-wrap gap-3">
          <span>
            <span className="text-muted-foreground">Before: </span>
            {formatUsdCompact(beforeTotal)}
          </span>
          <span className="text-muted-foreground">→</span>
          <span>
            <span className="text-muted-foreground">After: </span>
            {formatUsdCompact(afterTotal)}
          </span>
          <span className={cn("font-medium", delta >= 0 ? "text-green-600" : "text-destructive")}>
            ({delta >= 0 ? "+" : ""}
